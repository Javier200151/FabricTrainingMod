...
public class InventoryUtil {
    ...
    public static boolean hasPlayerStackInInventoryWithoutNBT(PlayerEntity player, Item item) {
        for(int i = 0; i < player.getInventory().size(); i++) {
            ItemStack currentStack = player.getInventory().getStack(i);
            if (!currentStack.isEmpty() && currentStack.isItemEqual(new ItemStack(item)) && currentStack.getNbt() == null) {
                return true;
            }
        }
    ...
    public static int getFirstInventoryIndexWithoutNBT(PlayerEntity player, Item item) {
        for(int i = 0; i < player.getInventory().size(); i++) {
            ItemStack currentStack = player.getInventory().getStack(i);
            if (!currentStack.isEmpty() && currentStack.isItemEqual(new ItemStack(item)) && currentStack.getNbt() == null) {
                return i;
